...
const BUSINESS = {
  ...
  services:
    "Arreglos de Carpinteria, Pintado y Barnizado en General (Paredes Interiores, Exteriores, y Carpinteria)",
};
...
export async function GET(
  ...
  try {
    ...
    doc.setFont("helvetica", "normal");
    const issueDate = new Date(invoice.issue_date as string).toLocaleDateString(
      "es-ES",
    );
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(9);
    doc.text("CONCEPTO", margin + colConcepto / 2, y + 5.5, {
      align: "center",
    });
    doc.text("UNIDAD", margin + colConcepto + colUnidad / 2, y + 5.5, {
      align: "center",
    });
    doc.text(
      "PRECIO",
      margin + colConcepto + colUnidad + colPrecio / 2,
      y + 5.5,
      { align: "center" },
    );
    doc.text(
      "TOTAL",
      margin + colConcepto + colUnidad + colPrecio + colTotal / 2,
      y + 5.5,
      { align: "center" },
    );
    y += 8;
    ...
    const drawTableLines = (startY: number, endY: number) => {
      ...
      doc.line(
        margin + colConcepto + colUnidad,
        startY,
        margin + colConcepto + colUnidad,
        endY,
      );
      doc.line(
        margin + colConcepto + colUnidad + colPrecio,
        startY,
        margin + colConcepto + colUnidad + colPrecio,
        endY,
      );
    ...
    for (const item of items) {
      ...
      doc.text(
        String(item.unit || ""),
        margin + colConcepto + colUnidad / 2,
        y + 5,
        { align: "center" },
      );
      doc.text(
    ...
    if (laborTotal > 0 && rowCount < maxRows) {
      doc.rect(margin, y, contentWidth, rowHeight, "S");
      doc.setFont("helvetica", "bold");
      doc.text("TOTAL MANO DE OBRA", margin + colConcepto / 2, y + 5, {
        align: "center",
      });
      doc.setFont("helvetica", "normal");
      doc.text("-", margin + colConcepto + colUnidad / 2, y + 5, {
        align: "center",
      });
      doc.text("-", margin + colConcepto + colUnidad + colPrecio - 2, y + 5, {
        align: "right",
      });
      doc.text(laborTotal.toFixed(2), margin + contentWidth - 2, y + 5, {
        align: "right",
      });
      y += rowHeight;
      rowCount++;
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(9);
    doc.text("BASE IMPONIBLE", totalsX + (totalsWidth - 30) / 2, y + 5, {
      align: "center",
    });
    doc.rect(pageWidth - margin - 30, y, 30, 7, "S");
    doc.setFont("helvetica", "normal");
    doc.text(`${Number(invoice.subtotal).toFixed(2)} €`, valueX, y + 5, {
      align: "right",
    });
    y += 7;
    ...
    doc.rect(totalsX, y, totalsWidth - 30, 7, "F");
    doc.setFont("helvetica", "bold");
    doc.text(`IVA ${taxRate} %`, totalsX + (totalsWidth - 30) / 2, y + 5, {
      align: "center",
    });
    doc.rect(pageWidth - margin - 30, y, 30, 7, "S");
    doc.setFont("helvetica", "normal");
    doc.text(`${Number(invoice.tax_amount).toFixed(2)} €`, valueX, y + 5, {
      align: "right",
    });
    y += 7;
    ...
    doc.rect(totalsX, y, totalsWidth - 30, 7, "F");
    doc.setFont("helvetica", "bold");
    doc.text("TOTAL FACTURA - EUROS", totalsX + (totalsWidth - 30) / 2, y + 5, {
      align: "center",
    });
    doc.rect(pageWidth - margin - 30, y, 30, 7, "S");
    doc.text(`${Number(invoice.total).toFixed(2)} €`, valueX, y + 5, {
      align: "right",
    });
